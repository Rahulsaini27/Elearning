                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
                            ...
                            <div className="px-6 -mt-16 relative z-20">
                                ...
                                <div className="text-center">
                                    ...
                                    <p className="text-slate-600 mb-4">{user?.occupation}</p>

                                    <div className="flex justify-center space-x-4 mb-6">
                                        <div className="bg-indigo-600/10 text-indigo-600 px-3 py-1 rounded-full text-xs">
                                            {user?.education} & Tech Enthusiast
                                        </div>
                                        <div className="bg-indigo-600/10 text-indigo-600 px-3 py-1 rounded-full text-xs">
                                            {user?.totalAchievements || "0"} Achievements
                        ...
                        <div className="bg-white rounded-3xl shadow-2xl mt-8 p-6">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center">
                                    <Mail className="mr-3 text-slate-600" />
                                    <span className="text-slate-800">{user?.email}</span>
                                </div>
                                <div className="flex items-center">
                                    <Phone className="mr-3 text-slate-600" />
                                    <span className="text-slate-800">{user?.phone}</span>
                                </div>
                                <div className="flex items-center">
                                    <MapPin className="mr-3 text-slate-600" />
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="bg-white rounded-3xl shadow-2xl p-8">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-3xl font-bold text-slate-800 flex items-center">
                                    <Book className="mr-4 text-indigo-600" />
                                    My Courses
                                </h2>
                                <div className="bg-indigo-600/10 text-indigo-600 px-4 py-2 rounded-full">
                                    Total Courses: {user?.enrolledCourses.length}
                                </div>
                            </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                {user?.enrolledCourses.map((course) => (
                                    ...
                                    >
                                        {/* Subtle Shadow Effect */}
                                        <div className="absolute -inset-1 bg-indigo-600/20 rounded-2xl opacity-25 group-hover:opacity-50 transition duration-300 blur-md"></div>
                                        ...
                                        <div className="relative bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
                                            ...
                                            {/* Category Badge */}
                                            <div className="absolute top-4 right-4 bg-indigo-600/10 text-indigo-600 px-3 py-1 rounded-full text-xs font-medium z-10">
                                                {course.category}
                                            </div>
                                            ...
                                            <div className="p-6 pt-24 relative z-20">
                                                ...
                                                <p className={`text-sm text-slate-600 ${expandedCourses[course._id] ? '' : 'line-clamp-2'}`}>
                                                    {course.description}
                                                </p>

                                                <button
                                                    className="text-indigo-600 text-xs font-medium mt-2 flex items-center"
                                                    onClick={() => toggleCourseDescription(course._id)}
                                                >
                                                    {expandedCourses[course._id] ? 'Show Less' : 'Read More'}
                                                    {expandedCourses[course._id] ? <ChevronUp className="ml-1 h-4 w-4" /> : <ChevronDown className="ml-1 h-4 w-4" />}
                                                ...
                                                <div className="flex justify-between items-center mt-6">
                                                    <div className="flex items-center space-x-2">
                                                        <div className="p-1.5 bg-indigo-600/10 rounded-full">
                                                            <Book className="h-4 w-4 text-indigo-600" />
                                                        </div>
                                                        <span className="text-xs text-slate-600">{course.lessons} Lessons</span>
                                                    </div>
                                                    <div className="flex items-center space-x-2">
                                                        <div className="p-1.5 bg-indigo-600/10 rounded-full">
                                                            <Video className="h-4 w-4 text-indigo-600" />
                                                        </div>
                                                        <span className="text-xs text-slate-600">{course.videos.length} Videos</span>
                                                    </div>
                                                </div>
                                                ...
                                                <button
                                                    ...
                                                    className="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg flex items-center justify-center mt-4 transition-all group"
                                                >
                                                    Continue Learning
                                                    <ArrowRight className="ml-2 h-4 w-4 transform group-hover:translate-x-1 transition-transform" />
                                                </button>
                        ...
                        <div className="bg-white rounded-3xl shadow-2xl p-8">
                            <h2 className="text-3xl font-bold text-slate-800 flex items-center mb-6">
                                <Calendar className="mr-4 text-indigo-600" />
                                Account Overview
                            </h2>
                            <div className="grid md:grid-cols-2 gap-6">
                                <div>
                                    <p className="text-slate-600 text-sm mb-2">Joined Date</p>
                                ...
                                <div>
                                    <p className="text-slate-600 text-sm mb-2">Total Learning Time</p>
                                    <p className="text-slate-800 font-medium">
                                        126 hours
                                    </p>
                                </div>
