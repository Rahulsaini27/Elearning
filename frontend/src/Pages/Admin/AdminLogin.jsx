  ...
  return (
    <div ref={formRef} className="relative">
        <div className="text-center">
          <h3 className="mt-4 text-2xl font-semibold text-gray-900">Admin Access</h3> {/* MODIFIED: text-white to text-gray-900 */}
          <p className="text-gray-600 text-sm">Log in to your administrator account</p> {/* MODIFIED: text-gray-300 to text-gray-600 */}
        </div>
        ...
        <form onSubmit={handleLogin} className="mt-6">
          ...
          <input type="password" name="password" value={formData.password} onChange={handleChange} placeholder="Password" className="w-full px-4 py-2 mt-3 rounded-lg bg-gray-100 text-gray-900 placeholder-gray-500 border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required /> {/* MODIFIED: Styles for white BG */}
          <div className="flex items-center justify-between mt-4">
            <button type="button" onClick={() => onSwitchForm("forgot")} className="text-sm text-blue-500 hover:underline">Forgot Password?</button> {/* MODIFIED: text-blue-300 to text-blue-500 */}
            <button type="submit" className="px-6 py-2 text-white bg-blue-600 rounded-lg shadow-md hover:bg-blue-700 transition-all" disabled={loading}>{loading ? "Signing In..." : "Sign In"}</button>
          </div>
        </form>
